...
func (s *ConfigService) GetNodeConfig(nodeId uint) (*[]byte, error) {
	data, err := s.SettingService.GetConfig()
	if err != nil {
		return nil, err
	}
...
func (s *ConfigService) StartCore() error {
	...
	if err != nil {
		...
		if failures >= 3 {
			logger.Warning("Core failed to start 3 times consecutively. Attempting database auto-rollback...")
			errRoll := database.RollbackToBackup(config.GetDBPath())
			if errRoll != nil {
				logger.Error("Database rollback failed: ", errRoll.Error())
			} else {
				logger.Info("Database successfully rolled back to last known good configuration. Retrying start...")
				startCoreMu.Lock()
	...
	// Start a timer to verify core stability and backup DB
	go func() {
		...
		if corePtr.IsRunning() {
			...
			consecutiveFailures = 0
			startCoreMu.Unlock()
			errBackup := database.BackupDB(config.GetDBPath())
			if errBackup == nil {
				logger.Info("Database successfully backed up as last known good configuration.")
			}
		}
	}()
