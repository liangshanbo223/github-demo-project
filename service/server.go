...
func (s *ServerService) GenKeypair(keyType string, options string) []string {
	...
	case "ca":
		return s.generateCAKeyPair()
	case "ca-server":
		return s.generateCAServerKeyPair(options)
	}

	return []string{"Failed to generate keypair"}
}

func (s *ServerService) generateCAKeyPair() []string {
	caCertPath := filepath.Join(config.GetDBFolderPath(), "ca.crt")
	caKeyPath := filepath.Join(config.GetDBFolderPath(), "ca.key")

	err := util.GenerateCACert(caCertPath, caKeyPath)
	if err != nil {
		return []string{"Failed to generate CA root cert: ", err.Error()}
	}

	certBytes, err := os.ReadFile(caCertPath)
	if err != nil {
		return []string{"Failed to read CA root cert: ", err.Error()}
	}

	return strings.Split(string(certBytes), "\n")
}

func (s *ServerService) generateCAServerKeyPair(serverName string) []string {
	if len(serverName) == 0 {
		serverName = "myserver"
	}
	caCertPath := filepath.Join(config.GetDBFolderPath(), "ca.crt")
	...
	if _, err := os.Stat(caCertPath); os.IsNotExist(err) {
		err = util.GenerateCACert(caCertPath, caKeyPath)
		if err != nil {
			return []string{"Failed to auto generate CA root cert: ", err.Error()}
		}
	}

	certPEM, keyPEM, err := util.GenerateServerCertByCA(caCertPath, caKeyPath, serverName)
	if err != nil {
		return []string{"Failed to generate server certificate by CA: ", err.Error()}
	}

	return append(strings.Split(string(certPEM), "\n"), strings.Split(string(keyPEM), "\n")...)
}
