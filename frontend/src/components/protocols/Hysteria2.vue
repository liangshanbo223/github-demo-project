...
export default {
  ...
  computed: {
    ...
    masqueradeType: {
      ...
      set(v:string) {
        if (v == '') {
          ...
          this.$props.data.masquerade = { type: v, url: "http://127.0.0.1:80", rewrite_host: true }
        } else {
    ...
    optionMasq: {
      ...
      set(v:boolean) { this.$props.data.masquerade = v ? { type: "proxy", url: "http://127.0.0.1:80", rewrite_host: true } : undefined }
    },
    optionMPort: {
