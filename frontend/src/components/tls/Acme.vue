<template>
  ...
  <!-- 实时日志弹窗 -->
  <v-dialog v-model="logDialog" width="700" persistent>
    <v-card class="rounded-lg bg-grey-darken-4 text-green-accent-3">
      <v-card-title class="d-flex justify-space-between align-center">
        <span>ACME 申请实时日志</span>
        <v-btn icon="mdi-close" variant="text" color="white" @click="closeLogDialog" :disabled="acmeRunning"></v-btn>
      </v-card-title>
      <v-divider color="white"></v-divider>
      <v-card-text style="background-color: #1a1a1a; font-family: monospace; height: 350px; overflow-y: scroll;" ref="logConsole">
        <div v-for="log, idx in logList" :key="idx" style="white-space: pre-wrap; line-height: 1.4;">
          {{ log }}
        </div>
      </v-card-text>
    </v-card>
...
export default {
  ...
  methods: {
    async startAcmeIssue() {
      if (this.domains.trim() === "") {
        push.error({ message: "请先填写需要签发的域名" })
        return
      }

      this.logList = ["[系统] 正在发起签发请求..."]
      this.logDialog = true
      this.acmeRunning = true

      let dnsProvider = ""
      let dnsParams: string[] = []
      if (this.acme.dns01_challenge) {
        dnsProvider = this.acme.dns01_challenge.provider
        const p = this.dnsProviders.filter(d => d.provider === dnsProvider)[0]
        if (p) {
          p.params.forEach(param => {
            const val = (this.acme.dns01_challenge as any)[param] || ""
            let envName = ""
            if (dnsProvider === "cloudflare") {
              if (param === "api_token") envName = "CF_Token"
              if (param === "zone_token") envName = "CF_Zone_ID"
            } else if (dnsProvider === "alidns") {
              if (param === "access_key_id") envName = "Ali_Key"
              if (param === "access_key_secret") envName = "Ali_Secret"
            } else if (dnsProvider === "acmedns") {
              if (param === "username") envName = "ACME_DNS_USER"
              if (param === "password") envName = "ACME_DNS_PASS"
            }
            if (envName) {
              dnsParams.push(`${envName}=${val}`)
            }
          })
        }
      }
      ...
      const baseApi = 'api'
      const res = await HttpUtils.post(`${baseApi}/acmeIssue`, {
        email: this.acme.email || "",
        domains: this.acme.domain,
        dnsProvider: dnsProvider,
        dnsParams: dnsParams
      })

      if (!res.success) {
        this.logList.push(`[错误] ${res.obj}`)
        this.acmeRunning = false
        push.error({ message: res.obj })
        return
      }

      this.logList.push("[系统] 后端任务已启动，正在连接日志流...")
    ...
    connectLogStream() {
      ...
      const baseApi = 'api'
      this.eventSource = new EventSource(`${location.origin}/${baseApi}/acmeLog`)
      
      this.eventSource.onmessage = (event) => {
        this.logList.push(event.data)
        this.$nextTick(() => {
          const consoleEl = this.$refs.logConsole as HTMLElement
          if (consoleEl) {
            consoleEl.scrollTop = consoleEl.scrollHeight
          }
        })

        if (event.data.includes("[ACME] 全流程已顺利完成！")) {
          push.success({ message: "证书申请成功并部署！" })
          this.acmeRunning = false
          this.eventSource?.close()
        }
        if (event.data.includes("证书申请执行失败") || event.data.includes("部署失败")) {
          push.error({ message: "证书签发失败" })
          this.acmeRunning = false
          this.eventSource?.close()
        }
      }
