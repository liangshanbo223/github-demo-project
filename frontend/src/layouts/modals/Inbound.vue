...
export default {
  ...
  methods: {
    async loadData(id: number) {
      ...
      if (this.inbound.node_id === undefined) {
        this.inbound.node_id = 0
      }
    ...
    updateData(id: number) {
      if (id > 0) {
        ...
        this.inbound = createInbound("direct",{ id: 0, tag: "direct-"+port ,listen: "::", listen_port: port, node_id: 0 })
    ...
    changeType() {
      ...
      // Use previous data
      const prevConfig = { id: this.inbound.id, tag: tag, listen: this.inbound.listen?? "::", listen_port: this.inbound.listen_port, node_id: this.inbound.node_id?? 0 }
      this.inbound = createInbound(this.inbound.type, this.inbound.type != this.inTypes.Tun ? prevConfig : { tag: tag, node_id: this.inbound.node_id?? 0 })
      if (this.HasInData.includes(this.inbound.type)){
        this.inbound.addrs = []
        this.inbound.out_json = {}
      } else {
