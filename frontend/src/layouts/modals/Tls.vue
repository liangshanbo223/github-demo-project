  ...
  methods: {
    ...
    saveChanges() {
      if (this.tlsType == 0 && this.usePath == 1) {
        const cert = this.certText.trim();
        const key = this.keyText.trim();
        
        if (cert && (!cert.startsWith("-----BEGIN CERTIFICATE-----") || !cert.endsWith("-----END CERTIFICATE-----"))) {
          push.error({ message: "证书格式错误，必须为 PEM 格式" })
          return
        }
        if (key && (!key.includes("-----BEGIN") || !key.includes("-----END"))) {
          push.error({ message: "私钥格式错误，必须为 PEM 格式" })
          return
        }
      }
    ...
    async genCA() {
      this.loading = true
      const msg = await HttpUtils.get('api/keypairs', { k: "ca" })
      this.loading = false
      if (msg.success) {
        push.success({ message: "自建 CA 根证书生成成功！已自动开启下载..." })
        window.open(location.origin + "/ca/download", "_blank")
      } else {
        push.error({ message: i18n.global.t('error') + ": " + msg.obj })
      }
    },
    async genServerCertByCA() {
      if (!this.inTls.server_name) {
        push.error({ message: "请先填写 SNI (域名)" })
        return
      }
      this.loading = true
      const msg = await HttpUtils.get('api/keypairs', { k: "ca-server", o: this.inTls.server_name })
      this.loading = false
      if (msg.success) {
        ...
        push.success({ message: "成功由自建 CA 签发该域名证书对！" })
      } else {
        push.error({ message: i18n.global.t('error') + ": " + msg.obj })
      }
    },
